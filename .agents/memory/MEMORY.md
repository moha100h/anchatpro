...
- [Plisio webhook body-parser conflict](plisio-webhook-body-parser.md) — must set `(req as any)._body = true` in raw-body middleware or express.json() overwrites/corrupts req.body on exhausted stream.
- [Plisio callback URL base derivation](plisio-callback-url-base.md) — derive returnBaseUrl from callbackUrl (strip /webhook/plisio), not getBaseUrl(), to avoid localhost success/fail redirects on VPS.
