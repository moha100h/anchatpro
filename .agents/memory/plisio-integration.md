...
- **Double-crediting race fix**: crediting used select-then-update (check `callbackVerified`, then later `UPDATE`), which is a TOCTOU race if Plisio delivers the same "completed" webhook twice concurrently — both requests could pass the check before either commits. Fixed by making the UPDATE itself the atomic guard: `UPDATE ... SET callbackVerified=true WHERE id=? AND callbackVerified=false RETURNING *`; only the request whose UPDATE actually returns a row is allowed to credit coins. Same pattern applied to the payment row's pending→approved transition. Verified with a real concurrent-call test (see plisio-verify-bug.md pattern) before relying on it.
