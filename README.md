...
اسکریپت فقط ۳ ورودی از شما می‌گیرد:
1. **توکن ربات** از [@BotFather](https://t.me/BotFather) — بلافاصله با تلگرام اعتبارسنجی می‌شود
2. **آیدی عددی ادمین** (Telegram ID) — و در صورت تمایل، آیدی ادمین دوم
3. **آدرس عمومی سرور / دامنه** — برای webhook درگاه‌های پرداخت *(اختیاری، بعداً هم می‌توان اضافه کرد)*

بقیه کارها کاملاً **خودکار** انجام می‌شود و نیاز به هیچ دخالت دستی نیست:

| مرحله | کار انجام‌شده |
|-------|----------------|
| **0** | به‌روزرسانی سیستم‌عامل (`apt/yum/dnf upgrade`) + نصب پیش‌نیازها (`curl`, `git`, `openssl`, `ca-certificates`) |
| **1** | دریافت و اعتبارسنجی توکن، آیدی ادمین‌ها و دامنه |
| **2** | نصب Node.js 22 (LTS) در صورت نبود |
| **3** | نصب pnpm |
| **4** | نصب و راه‌اندازی PostgreSQL + ساخت دیتابیس و کاربر با رمز تصادفی + تنظیم `pg_hba.conf` |
| **5** | ساخت فایل `.env` |
| **6** | نصب پکیج‌ها (`pnpm install`) |
| **7** | ساخت پروژه (esbuild) |
| **8** | اعمال کامل طرح دیتابیس — همه ۴۲ جدول (`drizzle-kit push`) + مایگریشن‌های ستونی |
| **9** | باز کردن پورت‌های فایروال (5000, 80, 443) در صورت فعال بودن ufw/firewalld |
| **10** | راه‌اندازی PM2 + فعال‌سازی auto-start در ریبوت |
| **11** | بررسی سلامت (health check) و نمایش لاگ در صورت مشکل |

> اگر سیستم‌عامل شما به‌روزرسانی لازم داشته باشد، اسکریپت خودش در مرحله ۰ این کار را انجام می‌دهد؛ نیازی به اجرای دستی `apt update && apt upgrade` نیست.
