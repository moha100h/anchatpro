...
export function registerGroupHandlers(bot: Bot<BotContext>) {
  ...
  bot.hears([/^👥 گروه/, /^👥 Anonymous Group/], async (ctx) => {
    ...
    // Notify existing members about new member (using i18n, per-member language)
    for (const memberId of members) {
      ...
      await bot.api
        .sendMessage(memberId, t(memberLang).memberJoined(myAlias, memberCount))
        .catch(() => {});
    }
  });
  ...
  bot.hears([/^🚪 خروج/, /^🚪 Leave Group/], async (ctx) => {
    ...
    // Get group and alias before leaving
    ...
    if (result.remaining < 2) {
      ...
      for (const memberId of members) {
        ...
        await bot.api
          .sendMessage(memberId, t(mLang).groupEnded, { reply_markup: mainMenuKeyboard(mLang) })
          .catch(() => {});
      }
    } else {
      // Notify remaining members (using per-member language)
      const members = await getGroupMembers(result.groupId);
      for (const memberId of members) {
        const mUser = await getUserByTelegramId(memberId);
        const mLang = (mUser?.language as "fa" | "en") ?? "fa";
        await bot.api
          .sendMessage(memberId, t(mLang).memberLeft(myAlias, result.remaining))
          .catch(() => {});
      }
    }
  });
